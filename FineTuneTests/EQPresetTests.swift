...
struct EQSettingsNormalizationTests {
    ...
    @Test("flat default EQ has no audible effect")
    func flatHasNoAudibleEffect() {
        let eq = EQSettings.flat
        #expect(!eq.isEnabled)
        #expect(!eq.hasAudibleEffect)
    }

    @Test("non-zero enabled EQ has audible effect")
    func nonZeroEnabledEQHasAudibleEffect() {
        let eq = EQSettings(bandGains: [0, 0, 3, 0, 0, 0, 0, 0, 0, 0], isEnabled: true)
        #expect(eq.hasAudibleEffect)
    }

    @Test("non-zero disabled EQ has no audible effect")
    func disabledEQHasNoAudibleEffect() {
        let eq = EQSettings(bandGains: [0, 0, 3, 0, 0, 0, 0, 0, 0, 0], isEnabled: false)
        #expect(!eq.hasAudibleEffect)
    }
}
...
struct EQSettingsCodableTests {
    ...
    @Test("Decoding with missing isEnabled defaults to false")
