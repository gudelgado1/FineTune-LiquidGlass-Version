...
struct DesignTokensTimingTests {
    ...
    @Test("VU meter update interval is ~120fps (ProMotion)")
    func vuMeterInterval() {
        // 1/120 s (~8.3 ms) so the meter runs full-speed on ProMotion; 60 Hz
        // displays cap naturally at 16.6 ms.
        let interval = DesignTokens.Timing.vuMeterUpdateInterval
        #expect(abs(interval - 1.0 / 120.0) < 0.001)
    }
