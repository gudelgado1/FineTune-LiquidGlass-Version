...
struct DesignTokensDynamicResolutionTests {
    ...
    func popupOverlay() {
        // A light white / dark black wash layered under the popup glass on the
        // pre-macOS-26 NSVisualEffectView fallback path.
    ...
    @Test("iconOnGlassEmphasis resolves correctly in light and dark")
    func iconOnGlassEmphasis() {
        expectColor(DesignTokens.Colors.iconOnGlassEmphasis,
                    equals: NSColor.black.withAlphaComponent(0.86),
                    in: Self.aqua)
        expectColor(DesignTokens.Colors.iconOnGlassEmphasis,
                    equals: NSColor.white.withAlphaComponent(0.96),
                    in: Self.darkAqua)
    }
