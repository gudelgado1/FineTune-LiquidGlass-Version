...
struct MediaKeyMonitorHandlerTests {
    ...
    @Test("volumeDown to 0 while unmuted reaches silence WITHOUT setting the mute flag")
    func volumeDownToZeroDoesNotAutoMute() {
        // Auto-mute-at-zero was removed deliberately: on DDC monitors it trapped
        // users (F11 → 0 set the mute flag, then F12 restored a stale savedVolume
        // instead of stepping back up). Volume at 0 with no mute flag is plain
        // "silent" and the next volumeUp cleanly steps from there. See the
        // volumeDown arm in `MediaKeyMonitor.handleCore`.
        let (monitor, _, _, _) = makeMonitor()
        let deviceID: AudioDeviceID = 1
        var writtenMute: Bool?
        var writtenVolume: Float?
        ...
        #expect(writtenMute == nil)  // mute flag left untouched
    }
