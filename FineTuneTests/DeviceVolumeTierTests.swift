...
final class MockAudioDeviceMonitor: AudioDeviceProviding {
    ...
    func removeOutputDevice(uid: String) {
        guard let device = devicesByUID.removeValue(forKey: uid) else { return }
        outputDevices.removeAll { $0.uid == uid }
...
final class MockDeviceVolumeProviding: DeviceVolumeProviding {
    ...
    func setDefaultDevice(_ deviceID: AudioDeviceID) -> Bool {
        guard let uid = deviceMonitor.device(for: deviceID)?.uid else { return false }
    ...
    func setDefaultInputDevice(_ deviceID: AudioDeviceID) -> Bool {
        ...
        onDefaultInputDeviceChanged?(uid)
        return true
    }
