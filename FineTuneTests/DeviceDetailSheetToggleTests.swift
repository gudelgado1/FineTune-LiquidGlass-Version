...
// AC #1–#5 for the binary software-volume control.
//
// Strategy: DeviceDetailSheet is a pure SwiftUI view; the load-bearing logic is
// the binding-to-tier mapping (useSoftwareBinding), the visibility predicates,
// and the copy for auto-software devices. These are static helpers and a
// synthesized Binding, so they are unit-testable without rendering.
//
...
@Suite("DeviceDetailSheet - binary software-volume control (AC #1-#5)")
@MainActor
struct DeviceDetailSheetToggleTests {
    ...
    @Test("AC #2: software control section remains visible when auto tier is already .software")
    func softwareSectionVisibleWhenAutoIsSoftware() {
        #expect(DeviceDetailSheet.shouldShowSoftwareControlSection(autoTier: .software) == true)
    }

    @Test("AC #2: auto software tier uses active status copy")
    func softwareAutoTierCopyShowsActiveStatus() {
        #expect(DeviceDetailSheet.softwareControlTitle(autoTier: .software).contains("active"))
        #expect(DeviceDetailSheet.softwareControlCallout(autoTier: .software).contains("software"))
    }
