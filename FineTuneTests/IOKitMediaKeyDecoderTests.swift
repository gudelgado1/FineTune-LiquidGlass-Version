...
struct IOKitMediaKeyDecoderTests {
    ...
    // MARK: - Transport keys (F7/F8/F9)

    @Test("PLAY key-down (keyType=16, F8) decodes to playPause")
    func playDown() {
        let result = decoder.decode(data1: (16 << 16) | 0x0A00)
        #expect(result == .playPause)
    }

    @Test("PLAY auto-repeat (keyType=16) decodes to nil (no flip-flop on hold)")
    func playRepeat() {
        let result = decoder.decode(data1: (16 << 16) | 0x0A01)
        #expect(result == nil)
    }

    @Test("NEXT key-down (keyType=17, F9) decodes to nextTrack")
    func nextDown() {
        let result = decoder.decode(data1: (17 << 16) | 0x0A00)
        #expect(result == .nextTrack)
    }

    @Test("PREVIOUS key-down (keyType=18, F7) decodes to previousTrack")
    func previousDown() {
        let result = decoder.decode(data1: (18 << 16) | 0x0A00)
        #expect(result == .previousTrack)
    }

    @Test("Transport keys are flagged isTransport; volume/mute are not")
    func isTransportFlag() {
        #expect(MediaKeyEvent.playPause.isTransport)
        #expect(MediaKeyEvent.nextTrack.isTransport)
        #expect(MediaKeyEvent.previousTrack.isTransport)
        #expect(!MediaKeyEvent.volumeUp(isRepeat: false).isTransport)
        #expect(!MediaKeyEvent.muteToggle.isTransport)
    }
