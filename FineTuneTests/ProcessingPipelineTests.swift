...
struct ProcessingChainTests {
    ...
    @Test("Unity volume without DSP skips limiter and remains bit-exact")
    func unityNoDSPBitExactPassthrough() {
        let frames = 256
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        fill(input, bufferIndex: 0, value: 1.0)
        ...
        let outData = output.data(at: 0)
        for i in 0..<(frames * 2) {
            #expect(outData[i] == 1.0, "Unity/no-DSP path should not soft-limit sample \(i)")
        }
    }
...
// MARK: - Audio Engine Stress Tests

@Suite("ProcessTapController — Audio Engine Stress")
struct AudioEngineStressTests {

    @Test("Unity passthrough stress remains bit-exact and faster than realtime")
    func unityPassthroughStressIsBitExactAndRealtimeSafe() {
        let frames = 512
        let iterations = 3_000
        let sampleRate = 48_000.0
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        fillStereoSignal(input, frames: frames, amplitude: 0.6)
        ...
        let outData = output.data(at: 0)
        for i in 0..<(frames * 2) {
            #expect(outData[i] == inData[i], "Unity/no-DSP fast path should be bit-exact at sample \(i)")
        }
        #expect(vol == 1.0, "Settled unity processing should not drift current volume")

        let renderedSeconds = Double(frames * iterations) / sampleRate
        #expect(
            elapsed < renderedSeconds * 0.5,
            "Unity path processed \(String(format: "%.2f", renderedSeconds))s of audio in \(String(format: "%.3f", elapsed))s"
        )
    }

    @Test("Full DSP stress remains finite, limited, and faster than realtime")
    func fullDSPStressRemainsFiniteLimitedAndRealtimeSafe() {
        let frames = 512
        let iterations = 1_500
        let sampleRate = 48_000.0
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        fillStereoSignal(input, frames: frames, amplitude: 0.35)

        let eq = EQProcessor(sampleRate: sampleRate)
        let eqSettings = EQSettings(
            bandGains: [3.0, -2.0, 1.5, 0.0, 5.0, 3.0, -1.5, 2.0, 0.0, -2.0],
            isEnabled: true
        )
        eq.updateSettings(eqSettings)

        var loudnessSettings = LoudnessEqualizerSettings()
        loudnessSettings.enabled = true
        let loudnessEqualizer = LoudnessEqualizer(settings: loudnessSettings, sampleRate: Float(sampleRate))

        let loudnessCompensator = LoudnessCompensator(sampleRate: sampleRate)
        loudnessCompensator.updateForVolume(0.25)
        #expect(loudnessCompensator.isEnabled, "Stress test expects loudness compensation to be active")

        var vol: Float = 0.1
        let elapsed = measureSeconds {
            for iteration in 0..<iterations {
                let target: Float = iteration.isMultiple(of: 2) ? 1.35 : 0.25
                processWithDefaults(
                    input: input,
                    output: output,
                    targetVol: target,
                    rampCoefficient: 0.04,
                    currentVol: &vol,
                    eqProc: eq,
        ...
        var maxAbs: Float = 0
        for i in 0..<(frames * 2) {
            let sample = outData[i]
            #expect(sample.isFinite, "Full DSP stress produced a non-finite sample at \(i): \(sample)")
            maxAbs = max(maxAbs, abs(sample))
        }
        #expect(maxAbs <= SoftLimiter.ceiling + 0.0001, "SoftLimiter should cap full-DSP stress output. Max: \(maxAbs)")
        #expect(vol.isFinite, "Volume ramp state should remain finite after stress")

        let renderedSeconds = Double(frames * iterations) / sampleRate
        #expect(
            elapsed < renderedSeconds,
            "Full DSP path processed \(String(format: "%.2f", renderedSeconds))s of audio in \(String(format: "%.3f", elapsed))s"
        )
    }

    @Test("Channel-remap stress keeps unused output channels silent")
    func channelRemapStressKeepsTrailingChannelsSilent() {
        let frames = 256
        let iterations = 2_000
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 6, frames: frames)])
        fillStereoSignal(input, frames: frames, amplitude: 0.5)
        ...
        for _ in 0..<iterations {
            processWithDefaults(
                ...
                output: output,
                targetVol: 0.8,
                rampCoefficient: 0.03,
                preferredStereoLeft: 2,
                preferredStereoRight: 3,
                currentVol: &vol
            )
        }

        let outData = output.data(at: 0)
        var routedEnergy: Float = 0
        for frame in 0..<frames {
            let base = frame * 6
            for channel in 0..<6 {
                let sample = outData[base + channel]
                #expect(sample.isFinite, "Channel-remap stress produced a non-finite sample at frame \(frame), channel \(channel)")
                if channel == 2 || channel == 3 {
                    routedEnergy += abs(sample)
                } else {
                    #expect(sample == 0, "Unused surround channel \(channel) should remain silent at frame \(frame)")
                }
            }
        }
        #expect(routedEnergy > 0, "Selected surround channels should receive routed stereo energy")
        #expect(abs(vol - 0.8) < 0.001, "Repeated ramp stress should settle near target volume. Got \(vol)")
    }
}

private func fillStereoSignal(
    _ abl: TestABL,
    frames: Int,
    sampleRate: Double = 48_000.0,
    amplitude: Float
) {
    let data = abl.data(at: 0)
    for frame in 0..<frames {
        let t = Double(frame) / sampleRate
        let left = amplitude * Float(sin(2.0 * Double.pi * 440.0 * t))
        let right = amplitude * Float(sin(2.0 * Double.pi * 997.0 * t))
        data[frame * 2] = left
