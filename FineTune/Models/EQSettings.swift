...
nonisolated struct EQSettings: Codable, Equatable {
    ...
    /// Whether this EQ actually changes the signal.
    ///
    /// A flat-but-enabled EQ is visually "on" in the UI, but running ten 0 dB
    /// biquad sections per app in the real-time audio callback is wasted CPU.
    /// The processor uses this to bypass DSP until a band has meaningful gain.
    var hasAudibleEffect: Bool {
        isEnabled && clampedGains.contains { abs($0) > 0.0001 }
    }
