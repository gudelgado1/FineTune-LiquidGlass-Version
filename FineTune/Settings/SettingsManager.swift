...
final class SettingsManager {
    ...
    /// Count of apps that have a *meaningful* (non-flat, enabled) EQ stored.
    /// Used by the Settings → Audio overview card to surface "how many apps
    /// am I currently shaping?". Stored apps that are merely enabled with all
    /// bands at 0 dB don't count — see `EQSettings.hasAudibleEffect`.
    var activeAppEQCount: Int {
        settings.appEQSettings.values.reduce(into: 0) { count, eq in
            if eq.hasAudibleEffect { count += 1 }
        }
    }
    ...
    // MARK: - Export / Import
    //
    // The persisted `Settings` struct is already `Codable`, so a snapshot is
    // just a JSON encoding of the same blob we save to disk every save tick.
    // Export gives users a portable backup; import replaces the current
    // in-memory state and schedules a save so the file on disk follows.
    //
    // Sound files (AutoEQ profiles imported from disk) live in
    // `AutoEQProfileManager`, not here — a full "everything backup" feature
    // would also need to round-trip those. For now we cover the
    // settings-only case, which is the bulk of what users want to preserve
    // (per-app volumes, EQ presets, device routings, hotkeys, preferences).

    /// Encodes the current settings to pretty-printed JSON. Suitable for
    /// `NSSavePanel`-driven export. Uses pretty printing so a curious user
    /// can read or hand-edit the file.
    ...
    /// Replaces the current settings from a JSON snapshot produced by
    /// `exportData()`. Throws on malformed input — callers should surface
    /// the error to the user instead of silently keeping the old state.
    /// On success, schedules a save so the new state hits disk on the
    /// next debounce tick.
    func importData(_ data: Data) throws {
        let imported = try JSONDecoder().decode(Settings.self, from: data)
        settings = imported
        scheduleSave()
        logger.info("Imported settings: \(self.settings.appVolumes.count) volumes, \(self.settings.appEQSettings.count) EQ settings, \(self.settings.appDeviceRouting.count) device routings")
    }
