...
struct AppRow: View {
    ...
    let onEQToggle: () -> Void
    let isFocused: Bool
    /// Whether this app is the current media-transport shortcut target.
    let isMediaTarget: Bool
    /// Toggles this app as the media-transport shortcut target.
    ...
    var body: some View {
        ExpandableGlassRow(isExpanded: isEQExpanded, isFocused: isFocused, onHoverChanged: { isRowHovered = $0 }) {
            ...
            HStack(spacing: DesignTokens.Spacing.sm) {
                ...
                // App name + chevron + optional routing subtitle.
                // Tapping the name area toggles the inline EQ panel, exactly
                // like tapping a device name opens the AutoEQ inspector.
                // The chevron rotates 90° when open and tints accent — same
                // motion vocabulary used throughout the popup.
                ...
                .onTapGesture {
                    // The parent's `toggleEQ(...)` already wraps the state
                    // mutation in withAnimation(.spring), so we just forward
                    // the toggle — no extra animation modifier needed here.
                    onEQToggle()
                }

                // Media-transport target marker — sits between the flexible name
                // column and the controls, so its hover-reveal collapse doesn't
                // shift the trailing controls.
                MediaTargetToggle(
                    isTarget: isMediaTarget,
                    isRowHovered: isRowHovered,
                    action: onToggleMediaTarget
                )
