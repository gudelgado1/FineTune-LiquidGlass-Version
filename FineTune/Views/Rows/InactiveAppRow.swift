    ...
    var body: some View {
        ExpandableGlassRow(isExpanded: isEQExpanded, isFocused: isFocused, onHoverChanged: { isRowHovered = $0 }) {
            ...
            HStack(spacing: DesignTokens.Spacing.sm) {
                ...
                // Tap the name to expand the EQ panel — same pattern as the
                // active AppRow, just inheriting the dimmed look of an
                // inactive (not currently producing audio) entry.
                ...
                .onTapGesture { onEQToggle() }

                // Media-transport target marker (works for paused apps too —
                // e.g. mark Spotify here so Play starts it).
                MediaTargetToggle(
