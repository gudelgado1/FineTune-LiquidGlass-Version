...
enum DesignTokens {
    ...
    enum Colors {
        ...
        /// Popup background overlay. Sits over NSVisualEffectView's `.menu`
        /// material. Kept near-zero so the native vibrancy does the heavy
        /// lifting — matching the Liquid Glass look where the glass is very
        /// transparent and luminous (macOS 26 Sound / Control Center style).
        ...
        /// Hairline border around the popup container (Liquid Glass edge ring).
        static let popupBorder = dynamicColor(
            name: "popupBorder",
        ...
        /// High-emphasis icon foreground on glass controls. Keeps icons visible
        /// against both the whiter light-mode glass and dark translucent surfaces.
        static let iconOnGlassEmphasis = dynamicColor(
            name: "iconOnGlassEmphasis",
    ...
    // MARK: - Typography (macOS 26 / Liquid Glass)
    //
    // Semantic SwiftUI fonts rather than fixed point sizes so the UI
    // automatically scales with Dynamic Type and matches platform conventions.
    //
    // macOS 26 reference sizes:
    //   .headline   → 13 pt semibold   .body     → 13 pt regular
    //   .callout    → 12 pt regular    .subheadline → 11 pt
    //   .footnote   → 10 pt            .caption  → 10 pt

    enum Typography {
        /// Uppercase section label (e.g. "APPS", "OUTPUT DEVICES").
        /// caption + semibold + tracking matches native macOS sidebar headers.
        static let sectionHeader = Font.system(.caption, weight: .semibold)

        /// Letter-spacing for section headers — tighter than before to feel
        /// lighter without losing readability.
        static let sectionHeaderTracking: CGFloat = 0.8

        /// Primary row label — app / device name at rest.
        static let rowName = Font.body

        /// Emphasised row label — default device or active app.
        static let rowNameBold = Font.headline          // 13 pt semibold

        /// Volume / gain percentage number.
        /// footnote size keeps it compact; monospacedDigit prevents layout shift.
        static let percentage = Font.system(.footnote, design: .monospaced)

        /// Secondary caption — hints, timestamps, small labels.
        static let caption = Font.caption

        /// Picker / menu button text.
        static let pickerText = Font.subheadline        // 11 pt

        /// EQ frequency axis labels — kept at 9 pt because the graph is dense.
        static let eqLabel = Font.system(size: 9, weight: .medium, design: .monospaced)

        /// AutoEQ profile card name.
        static let cardProfileName = Font.system(.callout, weight: .semibold)

        /// AutoEQ card sub-label (source / measuredBy).
        static let cardSource = Font.caption

        /// Settings section / card header.
        static let cardHeader = Font.headline           // 13 pt semibold

        /// Settings row helper description.
        static let rowDescription = Font.footnote       // 10 pt
    }
    ...
    enum Timing {
        /// VU meter minimum update interval. With `TimelineView(.animation)`
        /// this is a floor — the timeline drives at display refresh (60 Hz on
        /// regular displays, up to 120 Hz on ProMotion), capped here.
        /// 1/120 s = ~8.3 ms lets ProMotion run full speed; on 60 Hz displays
        /// the system naturally caps at 16.6 ms regardless.
        static let vuMeterUpdateInterval: TimeInterval = 1.0 / 120.0
    ...
    enum Links {
        ...
        static let support = URL(string: "https://github.com/gudelgado1/FineTune-LiquidGlass-Version")!

        /// Project license on GitHub
        static let license = URL(string: "https://github.com/gudelgado1/FineTune-LiquidGlass-Version/blob/main/LICENSE")!
    }
}
