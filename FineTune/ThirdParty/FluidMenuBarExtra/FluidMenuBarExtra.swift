...
//
//  Inlined into FineTune from upstream package
//  (https://github.com/wadetregaskis/FluidMenuBarExtra) so the project owns
//  the source and can apply local patches (notably the transparent panel
//  hosting view for Liquid Glass). `import Combine` was added because the
//  ObservableObject conformance below needs it explicitly when compiled as
//  part of the main target (vs. transitively via SwiftUI in the package).
...
/// State container for `FluidMenuBarExtra`. Defined at file scope (NOT nested
/// inside the generic struct) because the Swift 6.3 SIL EarlyPerfInliner
/// crashes on `isCallerAndCalleeLayoutConstraintsCompatible` when trying to
/// inline the implicit `deinit` of a class nested inside a generic struct
/// with `@StateObject` storage. The crash reproduces on both arm64 and
/// x86_64 Release builds with `-O`. Hoisting the class to file scope removes
/// the residual generic context the optimizer was tripping over.
///
/// `final` because we never subclass it; `@MainActor` because it carries
/// AppKit state (`FluidMenuBarExtraStatusItem`) — the `-default-isolation`
/// flag already implies main-actor here, but being explicit makes intent
/// clear and unblocks future audits.
@MainActor
private final class FluidMenuBarExtraState: ObservableObject {
    var statusItem: FluidMenuBarExtraStatusItem? = nil
}

public struct FluidMenuBarExtra<Content: View>: Scene {
    @StateObject private var state = FluidMenuBarExtraState()
