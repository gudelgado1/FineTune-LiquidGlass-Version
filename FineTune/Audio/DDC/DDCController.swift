...
final class DDCController {
    ...
    private func probe() {
        ...
        // Thread-safety: this closure runs on ddcQueue (non-actor context).
        // Safety is maintained by two rules:
        //   1. `logger` is captured by value before dispatch (Logger is Sendable).
        //   2. `getCoreAudioOutputDevices()` is nonisolated — it reads only
        //      CoreAudio APIs and carries no MainActor state.
        //   3. All mutations of @MainActor-isolated properties (services,
        //      ddcBackedDevices, cachedVolumes, onProbeCompleted) are dispatched
        //      back via `Task { @MainActor in }` at the end of the closure.
        // If the project moves to Swift 6 strict concurrency, restructure probe()
        // to pass a @Sendable completion handler instead of capturing self.
    ...
    // MARK: - Lifecycle Observers (display change, sleep/wake)

    /// DDC/I2C service handles (`IOAVService` CFTypeRefs) go stale across sleep/wake
    /// on Apple Silicon — macOS tears down and recreates the `DCPAVServiceProxy`
    /// IORegistry entries when a display power-cycles. Monitors also frequently reset
    /// or forget their audio volume on power-up, leaving FineTune's cached volume out
    /// of sync with the panel (writes to the dead handle fail silently, so the app and
    /// the monitor drift apart).
    ///
    /// Each of these notifications schedules a coalesced re-probe, which rebuilds fresh
    /// service handles and re-applies the persisted volume to the display (step 5 of
    /// `probe()`), then refreshes the UI via `onProbeCompleted`. We watch three signals
    /// because they don't all fire in every scenario:
    ///   • `didChangeScreenParametersNotification` — resolution / arrangement changes
    ///   • `screensDidWakeNotification` — a display woke (DPMS) while the Mac stayed awake
    ///   • `didWakeNotification` — the whole system woke from sleep
    private func setupLifecycleObservers() {
        func add(_ name: Notification.Name, on center: NotificationCenter, _ handler: @escaping () -> Void) {
        ...
        add(NSApplication.didChangeScreenParametersNotification, on: .default) { [weak self] in
            self?.scheduleReprobe(after: 3.0, reason: "display configuration changed")
        }
        add(NSWorkspace.screensDidWakeNotification, on: workspace) { [weak self] in
            self?.scheduleReprobe(after: 3.0, reason: "displays woke")
        }
        add(NSWorkspace.didWakeNotification, on: workspace) { [weak self] in
            self?.scheduleReprobe(after: 3.0, reason: "system woke from sleep")
        }
    }

    /// Coalesces rapid re-probe triggers (wake + screen-params often arrive together)
    /// into a single delayed probe. The delay gives a just-woken monitor time to bring
    /// its DDC/I2C endpoint back online before we read EDID and re-apply volume; the
    /// retry loop inside `DDCService.writeVCP` covers a monitor that's still groggy.
    private func scheduleReprobe(after delay: TimeInterval, reason: String) {
        probeWorkItem?.cancel()
        let item = DispatchWorkItem { [weak self] in
            Task { @MainActor [weak self] in
                guard let self else { return }
                self.logger.debug("Re-probing DDC: \(reason)")
                self.probe()
            }
        }
        probeWorkItem = item
