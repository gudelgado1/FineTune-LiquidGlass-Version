...
final class DeviceVolumeMonitor: DeviceVolumeProviding {
    ...
    /// Tasks driving the three system-level property streams (default output,
    /// system output, default input). Cancelling these tasks unregisters the
    /// underlying Core Audio listeners automatically via the AsyncStream's
    /// `onTermination` handler. Migrated from the old block+address pattern
    /// to remove manual remove-listener bookkeeping at shutdown.
    private var defaultOutputDeviceTask: Task<Void, Never>?
    private var systemOutputDeviceTask: Task<Void, Never>?
    private var defaultInputDeviceTask: Task<Void, Never>?
    ...
    // System-level property addresses are now imported from
    // `AudioObjectPropertyAddress` typed constants (.defaultOutputDevice,
    // .defaultSystemOutputDevice, .defaultInputDevice). The remaining device-
    // scope addresses (volume, mute) still live as instance vars because
    // they're shared by add/remove call sites further down the file.
    ...
    func start() {
        guard defaultOutputDeviceTask == nil else { return }
        ...
        // Default output device changes — AsyncStream over the system property.
        // Cancelling the task auto-removes the listener (see propertyChanges).
        defaultOutputDeviceTask = Task { @MainActor [weak self] in
            for await _ in AudioObjectID.system.propertyChanges(for: .defaultOutputDevice) {
                self?.handleDefaultDeviceChanged()
            }
        }

        // System output device changes (the one that plays UI sounds).
        systemOutputDeviceTask = Task { @MainActor [weak self] in
            for await _ in AudioObjectID.system.propertyChanges(for: .defaultSystemOutputDevice) {
        ...
        defaultInputDeviceTask = Task { @MainActor [weak self] in
            for await _ in AudioObjectID.system.propertyChanges(for: .defaultInputDevice) {
    ...
    func stop() {
        ...
        // Cancelling each task triggers `propertyChanges`'s onTermination,
        // which unregisters the underlying Core Audio listener. No manual
        // AudioObjectRemovePropertyListenerBlock calls needed.
        defaultOutputDeviceTask?.cancel()
        defaultOutputDeviceTask = nil
        systemOutputDeviceTask?.cancel()
        systemOutputDeviceTask = nil
        defaultInputDeviceTask?.cancel()
        defaultInputDeviceTask = nil
    ...
    private func clampedVolume(_ volume: Float) -> Float {
        // Snap to 0 only at genuinely-silent levels (~-80 dB). The previous
        // 0.01 threshold (-40 dB) was too aggressive for software tier where
        // perceptual `x²` mapping takes slider 6.25 % → gain 0.39 %, which
        // was being clamped back to 0 — leaving F12 from a silent state
        // unable to climb (the popup stayed at 0 % even as the HUD reported
        // the intended step). -80 dB is below conversational hearing and
        // still rounds away genuine HAL read noise.
        volume < 0.0001 ? 0 : volume
    }
    ...
    func setVolume(for deviceID: AudioDeviceID, to volume: Float) {
        ...
        case .hardware:
            ...
            if !success {
                logger.warning("HAL write failed for device \(deviceID) — UI will still reflect intended value")
            }
            // Always reflect the user's intent in `volumes` so the popup
            // slider doesn't desynchronize from the HUD. If the underlying
            // HAL write failed, the warning above is the only signal — the
            // alternative (leaving `volumes` stale) breaks media-key UX
            // because subsequent presses compute steps from the wrong base.
    ...
    func setMute(for deviceID: AudioDeviceID, to muted: Bool) {
        ...
        case .ddc:
            ...
            if let ddcController {
                ...
                muteStates[deviceID] = muted
                // Note: we deliberately do NOT update `volumes[deviceID]` here.
                // On mute, the UI's "visible" volume should stay where the
                // user last set it (the slider doesn't jump to 0). On unmute,
                // any subsequent media-key event reads from the updated
                // `cachedVolumes` via the next `setVolume` call.
            } else {
