...
extension AudioDeviceID {
    private nonisolated static let outputStreamDirection: UInt32 = 0
    ...
    private nonisolated func readStreams(scope: AudioObjectPropertyScope) throws -> [AudioObjectID] {
        var address = AudioObjectPropertyAddress(
            mSelector: kAudioDevicePropertyStreams,
            mScope: scope,
            mElement: kAudioObjectPropertyElementMain
    ...
    nonisolated func firstOutputStreamIndex() throws -> UInt {
    ...
    nonisolated func preferredStereoChannelIndices() -> (left: Int, right: Int) {
        var address = AudioObjectPropertyAddress(
